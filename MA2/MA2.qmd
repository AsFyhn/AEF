...
crsp_monthly = (pd.read_sql_query(
    sql="SELECT permno, month, ret_excess, mktcap, mktcap_lag, exchange FROM crsp_monthly",
    con=tidy_finance,
    parse_dates={"month"})
)
...
We report the average market cap and 12-month momentum for each decile portfolio in @tbl-alphasacross. Here, we equal-weight the weights of the stocks in the portfolios. Unsurprisingly, the table shows that the average market momentum increases with the momentum decile, but also that the average market capitalization is increasing in the momentum decile until the 7th decile.
...
merged = pd.merge(
  average_return,
  Alphas_excessret.drop(columns=['std_err', 'Alpha', 't-statistic', 'p-value']),
  on='Decile',
  how='outer'
  )
...
# select only stock that are on NEW YORK STOCK EXCHANGE
ml_crps = ml_crps.loc[ml_crps['exchange']=='NYSE']
```
...
plot_df['Sample'] = 'Full'
plot_df = pd.concat([plot_df,plot_df.loc[plot_df['RMSE']<0.7].assign(Sample='Zoom')])
