...
```{python}
# create a dataframe with lagged ex
r_df = pd.concat([ml_crps.groupby('permno')['ret_excess'].shift(i) for i in range(2,62)],axis=1)
r_df.columns = [col+f'_lag_{i+2}' for i, col in enumerate(r_df.columns.values)]
r_sq_df = r_df**2
...
(
  ggplot(pd.concat([plot_r_df, plot_rsq_df]), 
         aes(x="lag", y="value")) + 
  geom_col()  +
  facet_wrap("model") +
  ...
  theme(legend_position="none") + 
  theme_minimal()  +
  scale_fill_manual(["red", "blue"])  # Customize colors here
  )
```