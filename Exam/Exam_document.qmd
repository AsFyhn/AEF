...
# Exercise 3
In this exercise we compare the portfolios in Problem 1 and 2 with the naive portfolio and the efficient portfolio. The efficient portfolio is calculated as the minimum variance portfolio (MVP) that delivers 2 times the expected return of MVP. In order to calculate the weights, we need estimate of the expected returns, $\tilde{\mu}$, and the variance-covariance matrix, $\tilde{\Sigma}$. We estimate $\tilde{\mu}$ as the sample average of past returns, while using Leidot-Wolf shrinkage estimator for $\tilde{\Sigma}$. It is calculated as: 
...
To evaluate the performance of the four portfolios, we compute portfolio returns in the out-of-sample period from January 2016 until December 2020. 
...
We keep the portfolio weights constant throughout the out-of-sample period and do not empirically analyze how rebalancing and transaction costs would have affected returns of the portfolios. 
...
The Naive portfolio offers the best risk-adjusted return (Sharpe Ratio of 0.65). The low Sharpe ratio of the Fama French could indicate that it suffers from overfitting regression coefficient, where the parametric portfolio delivers a higher Sharpe-ratio because the estimation uncertainty of the parametrized portfolio is much smaller compared to the Fama French.
...
The implemented backtest strategy lacks several aspects of reality for instance reblancing and associated transaction cost. For the Fama French portfolio, the changes in input variables out-of-sample would be known to an investor who might want to rebalance based on these updates. 
If we assume constant realistic transactioncost, we expect the FF portfolio perform even worse, since it likely to have high very turnover. In contrast, we expect the PP portfolio normally have a lower turnover compared to FF since it is less sensitive to changes in a single firm's characteristics. We expect the efficient portfolio to have a little higher turnover than the PP. The naive portfolio will likely prove even more optimal due to low need for turnover.

The portfolio weights for each strategy are presented in @fig-portfolioweights.
The FF portfolio involves large variance in the weights. This is intuitive due to the estimation method that takes every firm's characteristics into account. The portfolio weights of the parametric portfolio are more balanced with one stock being shorted more than $10%$. The efficient portfolio also has relatively large variance in portfolio weights. 
