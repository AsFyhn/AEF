...
# Exercise 3
To evaluate the performance of the four portfolios, we compute portfolio returns in the out-of-sample period from January 2016 until December 2020, based on the December 2015 portfolio weights. As such, we keep the portfolio weights constant throughout the out-of-sample period and do not empirically analyze how rebalancing and transaction costs would have affected returns of the portfolios. The annualized returns, risk, and Sharpe Ratios are reported in @tbl-performancereport.
...
```
The Fama French portfolio policy involves going long or short stocks with up to 50% of the combined portfolio value. The Fame French portfolio is more sensitive to changes in firm characteristics, because it has a different estimate for each firm. This sensitivity leads to frequent and significant rebalancing, incurring high transaction costs and potentially diminishing returns. In contrast, the parametric portfolio requires less rebalancing and is therefore expected to perform better in the presence of transaction costs. We see in @fig-portfolioweights that the portfolio weights of the parametric portfolio are more balanced with only one stock being shorted significantly. Similarly, the naive portfolio is anticipated to have an advantage over the Fama French portfolio and the efficient portfolio when accounting for transaction costs.