...
We start by splitting the dataset into two datasets. One with all observations before 2016 and one with all observations after 2016. The first dataset will used throughout the study as a training dataset, and the second dataset will be used to evaluate the out-of-sample performance of the portfolios. Both datasets contain information for the same `{python} N` companies. The training dataset has `{python} T` monthly observations for each company, and the performance evaluation dataset has `{python} T_test` monthly observations. 
...
To examine the performance of the parametric portfolio, we start by estimating $\theta$ using the same approach as in Brandt, et. al (2009). Specifically, we solve the following optimization problem:
