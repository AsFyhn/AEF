...
We estimate the parameters of $\mathbf{A}_i$ and $\mathbf{B}_i$ of the following regression mode:
$$
\mathbf{r}_i = \mathbf{A}_i + \mathbf{F} \mathbf{B}_i + \mathbf{E}_i
$$
where $\mathbf{r}_i$ is the $T \times 1$ vector of excess returns for firm $i$ over time $t$. $\mathbf{A}_i$ is a $T \times 1$ vector representing the intercept ($\alpha$) for firm $i$. $\mathbf{F}$ is the $T \times 3$ matrix of factor returns over time $t$, where each column represents the factor returns, $r_{m,t}, r_{m,t}, r_{m,t}$. $\mathbf{B}_i=[\beta_i^m, \beta_i^{smb}, \beta_i^{hml}]$ is the vector of betas for firm $i$. Lastly, $\mathbf{E}_i$ is the $T \times 1$ vector of error terms for firm $i$ over time $t$.
...
where $\mathbf{\bar{F}}^{FF}$ represents the average of the factor returns over time, and $\hat{\Sigma}^{FR}$ is the covariance matrix of factor returns, and $\Sigma_{\epsilon}$ is the covariance matrix of the residuals. 
<!-- 
...
which has the analytical solution $\omega^* = \frac{1}{\gamma} (\Sigma^{-1}  - \frac{1}{\iota \Sigma^{-1} \iota} \Sigma^{-1}\iota \iota'\Sigma^{-1} )\mu + \frac{1}{\iota \Sigma^{-1} \iota} \Sigma^{-1}$

We create a portfolio with the optimal weights using all available data in the training dataset. The out-of-sample performance will be reported in Exercise 3. 
...
Where they use CRRA utility throughout the paper. They parametrize the portfolio weights directly, which reduces dimensionality exponentially compared to the two-step procedure in Problem 1. As such, the compounding of errors that can occur in the two-step procedure are minimized, reducing problems with overfitting and high variance or imprecise coefficient estimates. Furthermore, parametrizing portfolio weights, instead of the two-step procedure in Problem 1, reduces the computational requirements significantly. The computational requirements only grow with the number of characteristics and not with the number of stocks. 
...
To investigate the parametric portfolio policies further, we estimate $\theta$ on the dataset provided for the exam using the same approach as in Brandt, et. al (2009). Specifically, we solve the following optimization problem:
<!-- 
...
In exercise 3, we evaluate the performance of the paramatric portfolio. To do this, we need to calculate the portfolio policy weights in December 2015 $(t_{end})$. This is done by solving the equation:
$$\omega^{PP}_{i,t_{end}}=\frac{1}{N}+\frac{1}{N}\hat{\theta}'x_{i,t_{end}},$$
where $\hat{\theta}$ is the estimated values presented in @tbl-thetas. The optimal portfolio weights, $w^{PP}_{i,t_{end}}$, are presented in @fig-portfolioweights.

# Exercise 3
In this exercise we compare the portfolios in Problem 1 and 2 with the naive portfolio and the efficient portfolio. The efficient portfolio is calculated as...

We estimate $\tilde{\mu}$ as given sample average returns:
$$\tilde{\mu}=\frac{1}{T}\sum^T_{t=1}r_t$$
The co-variance matrix is estimated with the Ledoit-Wolf shrinkage estimator:
$$ 
\hat{\Sigma}^{LW} = \alpha \hat{\Sigma}_{\text{target}} + (1-\alpha) \hat{\Sigma}_{\text{sample}}
$$
...
To evaluate the performance of the four portfolios, we compute portfolio returns in the out-of-sample period from January 2016 until December 2020. The performance of the Fama French portfolio and the Parametric portfolio are based on the December 2015 portfolio weights. As such, we keep the portfolio weights constant throughout the out-of-sample period and do not empirically analyze how rebalancing and transaction costs would have affected returns of the portfolios. 
...
The Naive portfolio offers the highest mean return (13.57) and the best risk-adjusted return (Sharpe Ratio of 0.65) despite high volatility. Risk-averse investors might prefer the efficient portfolio, which provides a decent mean return (8.67) with lower volatility (17.16 standard deviation). The performance of the Fama French and the parametric portfolio policies shows, that we are not successful in creating profitable strategies in Problem 1 and 2.

Our backtest strategy only uses the data that is available at the time of the portfolio creation. This means that we do not use any future data to estimate the optimal portfolios, and therefore no look-ahead bias is present in our backtest. However, the Fama French portfolio comes with significant implementation shortfalls. This is due to the exponential increase in parameters as the number of stocks increase, requiring increasing computational power, and yielding noisy and unstable results. Kan and Zhou (2007) show that estimation and model uncertainty has huge implications for the optimal portfolio choice. As such, the estimation uncertainty of the Fama French portfolio is likely the reason for its poor performance in our study. When uncertainty is large enough, the naive portfolio can be optimal which is exactly the case in our study. The estimation uncertainty of the parametrized portfolio is much smaller compared to the Fama French, and the low amount of parameters makes the policiy computationally easy to implement in the real world even for a large amount of stocks. Furthermore, the need for constantly rebalancing the portfolio is reduced (Brandt, et. al (2009)). In the following, we discuss what implications transaction costs could have on the performance of our portfolios, if the portfolio weights are updated throughout the out-of-sample period. The portfolio weights for each strategy are presented in @fig-portfolioweights.
...
```
The Fama French portfolio policy involves going long or short stocks with up to 50% of the combined portfolio value. The Fame French portfolio is more sensitive to changes in firm characteristics, because it has a different estimate for each firm. This sensitivity leads to frequent and significant rebalancing, incurring high transaction costs and potentially diminishing returns. In contrast, the parametric portfolio normally have a lower turnover rate and is therefore expected to perform better in the presence of transaction costs. We see in @fig-portfolioweights that the portfolio weights of the parametric portfolio are more balanced with only one stock being shorted significantly. Similarly, the naive portfolio is anticipated to have an advantage over the Fama French portfolio and the efficient portfolio when accounting for transaction costs.