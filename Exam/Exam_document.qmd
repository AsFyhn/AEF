...
$$
where $\mathbf{r}_i$ is the $T \times 1$ vector of excess returns for firm $i$ over time $t$. $\mathbf{A}_i$ is a $T \times 1$ vector representing the intercept for firm $i$. $\mathbf{F}$ is the $T \times 3$ matrix of factor returns over time $t$, where each column represents the factor returns, $r_{m,t}, r_{m,t}, r_{m,t}$. $\mathbf{B}_i=[\beta_i^m, \beta_i^{smb}, \beta_i^{hml}]$ is the vector of betas for firm $i$. Lastly, $\mathbf{E}_i$ is the $T \times 1$ vector of error terms for firm $i$ over time $t$.
