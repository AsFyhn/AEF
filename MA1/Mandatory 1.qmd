...
To calculate the Sharpe ratio we use the formula: $Sharpe=\frac{return-0}{standarddeviation}$. The stock with the highest Sharpe ratio is Apple (AAPL), which can also be seen from the output of our code:
```{python}
