...
However, the efficient tangent portfolio has the maximum attainable Sharpe ratio of 1.38 when comprising a portfolio of only the Dow Jones 30 index. The Sharpe Ratio is significantly higher than that of AAPL, which makes sense as AAPL is only one single stock. AAPL is included in the efficient tangent portfolio, so we could at least get that Sharpe Ratio. Instead,the efficient tangent portfolio optimizes the risk/return tradeoff accross all 27 stocks.
...
def do_single_simulation():
  ...
  sim_tan_weights = calc_eff_tang_port(x=eff)
